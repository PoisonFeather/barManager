...
export function AnalyticsSection({ barId }: AnalyticsSectionProps) {
  ...
  // Admin module active view
  const [adminView, setAdminView] = useState<"financial" | "wait-times">("financial");

  const [data, setData] = useState<{ summary: any; topProducts: any[]; zReport?: any[] } | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!barId || adminView !== "financial") return;
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-24 md:pb-8 w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-10 border-b border-zinc-200 dark:border-zinc-800 pb-6">
        <div>
          ...
          <div className="flex gap-4 mt-4">
            <button
              ...
              className={`text-xs font-black uppercase tracking-widest pb-2 border-b-2 transition-colors ${adminView === "financial" ? "border-orange-500 text-orange-500" : "border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"
                }`}
            >
              📊 Raport Vanzari
            </button>
            <button
              onClick={() => setAdminView("wait-times")}
              className={`text-xs font-black uppercase tracking-widest pb-2 border-b-2 transition-colors ${adminView === "wait-times" ? "border-orange-500 text-orange-500" : "border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"
                }`}
            >
              ⏱️ Timpi Așteptare
            </button>
        ...
        {/* FILTRE PERIOADĂ - doar pt Financiar */}
        {adminView === "financial" && (
          <div className="flex bg-zinc-200 dark:bg-zinc-900 p-1 rounded-2xl w-full md:w-auto">
            {[
              ...
              <button
                ...
                className={`flex-1 md:flex-none px-4 py-3 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${period === tab.id
      ...
      {/* SECȚIUNE TIMPI AȘTEPTARE */}
      {adminView === "wait-times" && <WaitTimeSection barId={barId} />}
    </div>
  );
}